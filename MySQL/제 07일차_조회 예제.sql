...
select mc_name as 분류 , pr_title as 제품명, pr_price as 가격 from product
	...
	order by pr_price asc; -- 									 ㄴmc라는 임시 테이블 생성해야만 함
     
-- 제품별 판매량을 조회
select pr_title as 제품명, sum(ol_amount) as 판매량 from product
	right join product_option on pr_num= po_pr_num
    left join order_list on po_num = ol_po_num
	group by po_pr_num;
