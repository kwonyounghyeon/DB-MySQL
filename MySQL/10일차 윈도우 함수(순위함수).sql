/* 순위 함수 : row_number(), dense_rank(),rank(),ntitle()
    ...
    - dense_rank() : 같은 값이 있을 때 등수가 같음
					 같은 값이 있는 행 다음 등수는 같은 값이 있는 등수 +1
	- rank()	   : 같은 값이 있을 때 등수가 같음
				   : 같은 값이 있는 행 다음 등수는 같은 값이 있는 등수 + 같은 값이 있는 개수를 더함
	- ntile()	   : 등수를 총 n등까지 분할
...
pr_title as 제품명, pr_amount as 보유량 from product;
-- row_number()
select
	row_number() over(order by pr_price desc) as 가격순위,
pr_title as 제품명, pr_price as 제품가격 from product;
-- dense_rank()
select
...
select  row_number() over(order by pr_price desc)
pr_title,pr_price,
pr_price - lead(pr_price) over(order by pr_num asc) as 다음행값 from product;

select  row_number() over(order by pr_price desc)
pr_title,pr_price,
pr_price - lead(pr_price,1,0) over(order by pr_num asc) as 다음행값 from product;

select  row_number() over(order by pr_price desc)
pr_title,pr_price,
pr_price - lag(pr_price,1,pr_price) over(order by pr_num asc) as 이전행값 from product;

select  row_number() over(order by pr_price desc)
pr_title,pr_price,
pr_price - first_value(pr_price) over(order by pr_num asc) as 이전행값 from product;

-- 백분율
select  row_number() over(order by pr_price desc)
pr_title,pr_price,
	cume_dist() over(order by pr_num desc)*100 as 백분율 from product;

select pr_pc_num as 분류번호,avg(pr_price) as 평균가격 from product group by (pr_pc_num);
