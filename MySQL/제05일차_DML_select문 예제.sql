...
select pr_pc_num as 카테고리번호, count(pr_num) as '카테고리별 등록된 제품 수'
	from product 
    -- where count(pr_num)>=2 안됨 ! having절로
    group by pr_pc_num
...
-- 카테고리별 가장 비싼 제품의 가격을 조회하는 쿼리문 
select pr_pc_num as 카테고리번호, max(pr_price) as '가격'
	from product
    group by pr_pc_num;

-- 사용자별 총 구매 제품 개수를 조회(| 제품의 개수인 경우 count 사용)
select bu_me_id as '사용자',sum(bu_amount) as '총 구매 개수' from buy
	group by bu_me_id;
