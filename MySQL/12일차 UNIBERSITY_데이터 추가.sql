...
/* 학생 정보 추가를 위한 프로시저
 ...
 where substring(pr_num,5,3) like 160 and pr_name like '홍길동';
*/
...
BEGIN
    ...
    if _dep_num is not null then
		...
		set _professor_num = 
		(select pr_num from professor where substring(pr_num,5,3) like _dep_num and pr_name like _professor_name);
...
/*2022년 1학기 컴퓨터 개론을 수강하는 고길동 학생의 성적을 등록하려고 한다.(학번과 강좌번호로 작업)
 중간은 100, 기말은 90, 출석 100, 과제 100점이고, 비율은 중간 4, 기말 4, 출석1 과제 1로
 성적이 계산되어 총점에 저장
 학점은 100이하 90이상 A, 90미만 80이하 B, 80미만 70이하 C, 70미만 60이하 D, 나머지 F
 */
...
BEGIN
    ...
    set _total = new.sc_mid*0.4 + new.sc_final*0.4 +new.sc_project*0.1 +new.sc_attendance* 0.1;
    set _co_num = 
		(select co_num from course where co_st_num = _st_num and co_le_num = _le_num);
...
/* 트리거를 이용하여 성적을 추가하는 작업
	2022년 1학기 자바를 수강하는 고길동 학생의 성적을 등록하려고 한다(학번과 강좌 번호로 이용)
	중간은 100, 기말은 90, 출석 100, 과제 100점이고, 비율은 중간 4, 기말 4, 출석1 과제 1로
	성적이 계산되어 총점에 저장
	학점은 100이하 90이상 A, 90미만 80이하 B, 80미만 70이하 C, 70미만 60이하 D, 나머지 F
...
BEGIN 
    ...
    set new.sc_total = 
		new.sc_mid*0.4 +new.sc_final*0.4 +new.sc_project*0.1 +new.sc_attendance* 0.1;
...
END // 
DELIMITER ;
insert into score values(null,100,80,100,90,0,2);
